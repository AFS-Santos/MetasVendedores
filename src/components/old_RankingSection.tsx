...
function MarkupBonus({ vendedores, regras }: { vendedores: Vendedor[]; regras: Regras }) {
  ...
  const sorted = sortVendedores(elegiveis, 'markup')
  const foraTop3 = sorted.slice(3)
  const bonusVend = foraTop3.length ? foraTop3[0] : null

  if (!bonusVend && elegiveis.length <= 3) {
    return (
      <div className="mt-3 px-3 py-2 bg-gold/5 border border-gold/15 rounded-lg text-xs text-muted2">
        ⭐ Bônus Markup (R$ {Number(regras.bonusMk).toLocaleString('pt-BR')}): {elegiveis.length > 0 ? 'Todos elegíveis estão no Top 3' : 'Sem elegíveis'}
      </div>
    )
  }

  if (!bonusVend) return null

  return (
    <div className="mt-3 px-3 py-2 bg-gold/5 border border-gold/15 rounded-lg flex items-center gap-2.5">
      <div className="text-xs">
        <span className="text-gold font-semibold">⭐ Bônus Markup — R$ {Number(regras.bonusMk).toLocaleString('pt-BR')}</span>
        <span className="text-muted2 ml-1.5">Melhor markup fora do Top 3:</span>
      </div>
