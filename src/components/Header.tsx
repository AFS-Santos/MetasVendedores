...
export function Header({ onOpenAdd, onOpenRegras, onOpenMassEdit, onPdfRanking, onPdfPodio, onPdfFilial }: HeaderProps) {
  ...
  const handleSync = async () => {
    if (!sheetsUrl) { toast('Cole a URL do Apps Script!', 'err'); return }
    try {
      await syncFromSheets()
      const count = useDataStore.getState().vendedores.length
      toast(`✅ ${count} vendedores sincronizados!`)
    } catch { toast('Erro na sincronização', 'err') }
  }
    ...
    <header className="py-4 sm:py-6 border-b border-border mb-4 sm:mb-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="hidden lg:flex gap-2 items-center flex-wrap">
          ...
          <button onClick={handleSync} className={`${btnBase} border border-border text-silver hover:border-gold hover:text-gold`}>🔄 Sincronizar</button>
      ...
      {menuOpen && (
        <div className="lg:hidden mt-3 grid grid-cols-2 gap-2 animate-fade-in">
          ...
          <button onClick={() => { handleSync(); setMenuOpen(false) }} className={`${btnBase} border border-border text-silver py-2.5 text-center`}>🔄 Sincronizar</button>
